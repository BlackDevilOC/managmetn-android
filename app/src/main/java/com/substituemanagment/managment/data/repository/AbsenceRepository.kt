...
class AbsenceRepository @Inject constructor(
    ...
    fun getAbsencesByTeacher(teacherId: Int): Flow<List<Absence>> = 
        absenceDao.getAbsencesByTeacher(teacherId)
    
    fun getAbsencesByDate(date: String): Flow<List<Absence>> = 
        absenceDao.getAbsencesByDate(date)
    
    fun getAbsencesByStatus(status: AbsenceStatus): Flow<List<Absence>> = 
        absenceDao.getAbsencesByStatus(status)
    
    suspend fun insertAbsence(absence: Absence): Long = absenceDao.insertAbsence(absence)
