...
class SmsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * Send SMS to a single teacher
     */
    fun sendSms(context: Context, teacherName: String, phoneNumber: String, message: String) {
        viewModelScope.launch {
            _isLoading.value = true
            try {
                val success = SmsSender.sendSingleSms(
                ...
                if (success) {
                    ...
                    setError("Failed to send SMS to $teacherName")
                }
            } catch (e: Exception) {
                setError("Error sending SMS: ${e.message}")
            } finally {
                _isLoading.value = false
            }
        }
    ...
    fun loadData() {
        viewModelScope.launch {
            ...
            try {
                ...
                // Load selected teachers if they exist
    ...
    fun generatePreviewMessage(): String {
        val selectedTeacher = selectedTeachers.entries.firstOrNull { it.value }?.key
        if (selectedTeacher == null) {
            return "Select a teacher to preview the message"
        }

        val assignments = teacherAssignments[selectedTeacher] ?: return "No assignments found"
        val firstAssignment = assignments.firstOrNull() ?: return "No assignments found"

        return messageTemplate.value
            .replace("{substitute}", selectedTeacher)
            .replace("{class}", firstAssignment.className)
            .replace("{period}", firstAssignment.period.toString())
            .replace("{date}", firstAssignment.date)
            .replace("{day}", getCurrentDay())
            .replace("{original_teacher}", firstAssignment.originalTeacher)
    }
